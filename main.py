...
PROXY_URL = f"http://{WEBSHARE_USER}:{WEBSHARE_PASS}@31.59.20.176:6754" if WEBSHARE_USER else None
    ...
    try:
        ...
        if results:
            return {"results": "\n\n".join(results), "count": len(results),
                    "source": "duckduckgo", "source_label": "🦆 DuckDuckGo"}
    except:
        pass

    # ══ محاولة 2: DuckDuckGo HTML + Webshare ══
    try:
        search_url = f"https://html.duckduckgo.com/html/?q={urllib.parse.quote(req.query)}"
        async with httpx.AsyncClient(timeout=12, verify=False, headers=ua,
            proxies=proxies if proxies else {}) as c:
        ...
        if results:
            proxy_used = "مع Webshare" if proxies else "مباشر"
            return {"results": "\n\n".join(results), "count": len(results),
                    "source": "duckduckgo_html", "source_label": f"🦆 DuckDuckGo ({proxy_used})"}
    except:
        pass

    # ══ محاولة 3: Google + Webshare ══
    try:
        ...
        if results:
            proxy_used = "مع Webshare" if proxies else "مباشر"
            return {"results": "\n\n".join(results), "count": len(results),
                    "source": "google", "source_label": f"🔍 Google ({proxy_used})"}
    except:
        pass
